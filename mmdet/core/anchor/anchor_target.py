...
def anchor_target_single(flat_anchors,
    ...
    if len(pos_inds) > 0:
        ...
        if scale_balanced:
            mask = sampling_result.pos_assigned_gt_inds.detach().clone()
            scb_weights = sampling_result.pos_assigned_gt_inds.detach().clone().float()
            dt = dict()
            for tx in mask:
                x = int(tx)
                if x not in dt:
                ...
                dt[x] += 1.
            for x in dt:
                scb_weights[mask == x] = 3 * (math.log(dt[x]) + 1) / dt[x]
        if cfg.pos_weight <= 0:
            if scale_balanced:
                label_weights[pos_inds] = 1.0 * scb_weights
            else:
                label_weights[pos_inds] = 1.0
        else:
            if scale_balanced:
                label_weights[pos_inds] = cfg.pos_weight * scb_weights
            else:
